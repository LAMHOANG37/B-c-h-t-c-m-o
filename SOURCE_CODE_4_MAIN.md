# 📚 MÃ NGUỒN PHẦN 4: ĐIỂM KHỞI CHẠY CHÍNH (main.py)

Tập hợp mã nguồn khởi chạy 5 bot Discord, điều phối lệnh Slash, phiên hội bàn đa bot và vòng lặp giám sát.

---

## 📄 main.py

**Chức năng chính**: Điểm khởi động toàn bộ hệ thống: kết nối và đồng bộ lệnh cho 5 Bot Discord, xử lý sự kiện tin nhắn (on_message), điều hành phiên hội bàn đa bot (
un_multi_agent_roundtable), thực thi lệnh /kichban, /ve_anh, /hoiban, /quota, chia nhỏ tin nhắn dài (send_split_message), và chạy Web Dashboard.

**Các hàm/class quan trọng**:
- send_split_message(): Tự động chia nhỏ văn bản dài thành các đoạn < 1900 ký tự để không bao giờ bị lỗi giới hạn 2000 ký tự Discord
- 
un_multi_agent_roundtable(): Điều hành quy trình 6 bước họp bàn giữa 5 bot và tự động xuất kịch bản + vẽ ảnh 4K sau khi duyệt
- execute_script_generation(): Thực thi tạo kịch bản 4 bước và gửi file đính kèm
- execute_image_generation(): Thực thi render ảnh 4K và gửi ảnh trực tiếp vào chat
- execute_channel_audit(): Thực thi bóc tách toàn diện kênh YouTube
- handle_subagent_chat(): Xử lý trò chuyện riêng biệt cho từng sub-bot với tính cách và vai trò chuyên biệt
- quota_auto_alert_task(): Vòng lặp tự động phát cảnh báo khi quota API < 20%
- daily_scheduled_pipeline(): Lịch tự động chạy phân tích thị trường vào 19:30 hàng ngày
- main(): Khởi tạo kết nối đồng thời cho 5 bot Discord và khởi động Web Dashboard
